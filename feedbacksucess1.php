<?php
session_start();
?>


<!DOCTYPE html>
<html>
<head>
  <title>ONLINE AUCTION SYSTEM</title>
</head>
    <link rel="stylesheet" type="text/css" href="createacc.css">
    <body>
  <?php
  
       $con=mysqli_connect('127.0.0.1',"root","rumrum");
$db=mysqli_select_db($con,"online_auction2");
      
   

       /* if (!isset($_POST['productname'])||!isset($_POST['productdesc'])||!isset($_POST['startbidamt'])||!isset($_POST['strtdate'])||!isset($_POST['enddate']))
        {
            echo "<p> You have not entered all details.<br/>
             try again. </p>";
             exit;
        }/*/
		

     $iid=$_POST['itemid'];
	 $id=$_SESSION['id'];
	 $track=$_POST['feed2'];
	 
	mysqli_autocommit($con,FALSE);
		
		$sql1="SELECT final_pay from item where item_id='".$iid."' and bid='".$id."'";
		$result=mysqli_query($con,$sql1);
		$row=mysqli_fetch_array($result);
    if($row['final_pay']!=NULL)
	{
     $sql3 = "UPDATE item SET buyer_feedback='".$track."' WHERE item_id='".$iid."' and bid='".$id."'";
	 $result3=mysqli_query($con,$sql3);
	 
       
if ($result3) {
     echo "Feedback submitted";
}
else{
echo " error";
}

} else {
   echo "Either this item has not been evaluated or Payment have not been done yet or you are not the buyer";
 }
 mysqli_commit($con);
     mysqli_close($con);
		


        ?>
		<div class="clearfix">
		<form action="buyer.php" method="post">
        <button type="submit" formaction="buyer.php" class="signupbtn1">Go Back</button></form>
         
    </div>
        
   </body>
</html> 